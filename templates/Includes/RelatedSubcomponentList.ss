			...
			<div>
				<% if $Collection.ClassNames.Count %>
					<% loop $Collection.ClassNames %>
						<a href="$Top.Dashboard.join_links($Top.Dashboard.Link,edit,$Top.Dashboard.currentRecord.ID,relation,$Top.RelatedComponentName,$Top.Dashboard.relatedObject.ID,subrelation,$ComponentName)?type={$UrlParam}" class="btn btn-success btn-sm ml-2 float-right" role="button">New {$Title}</a>
					<% end_loop %>
				<% else %>
